...
\begin{itemize}
    ...
    \item 2. 齐次性：$\forall x \in X \ , \ \forall a \in \mathbb{R} \ or \ \mathbb{C} \ , \ \|ax\|=|a|\cdot\|x\| \ $
...
\[\|ax(t)\|=\mathop \text{sup}\limits_{t \in [a,b]}|ax(t)|=\mathop \text{sup}\limits_{t \in [a,b]}|a|\cdot|x(t)|=|a|\cdot\|x(t)\|\]
...
\item 2-2
\[\|a_nx_n-ax\| \leq \|a_nx_n-ax_n+ax_n-ax\| \leq \|a_nx_n-ax_n\|+\|ax_n-ax\|=|a_n-a|\cdot\|x_n\|+|a|\cdot\|x_n-x\| \to 0\]
\end{itemize}
\end{proof}
...
\[\|tx+(1-t)y\| \leq \|tx\|+\|(1-t)y\|=|t|\cdot\|x\|+|1-t|\cdot\|y\|<tr+(1-t)r=r \quad \Rightarrow \quad tx+(1-t)y \in B_r(0)\]
\end{proof}
...
\begin{definition}[无限维商空间]\label{definition:skj}
...
\item 2. 齐次性：
\[\|k\tilde{x}\|=\mathop \text{inf}\limits_{y \in k\tilde{x}=\tilde{kx}}\|y\|=\mathop \text{inf}\limits_{z \in \tilde{x}}\|kz\|=|k|\mathop \text{inf}\limits_{z \in \tilde{x}}\|z\|=|k|\cdot\|\tilde{x}\|\] 
\item 3. 三角不等式：
\begin{equation*}
\begin{aligned}
...
\begin{theorem}\label{theorem:skjwb}
...
\[x=\sum_{i=1}^{\infty}\xi_ie_i=\lim_{n \to \infty}\sum_{i=1}^{n}\xi_ie_i \quad \Leftrightarrow \quad \left\|x-\sum_{i=1}^n\xi_ie_i\right\|=\left\|\sum_{i=n+1}^{\infty}\xi_ie_i\right\| \rightarrow 0 \quad (n \rightarrow 0) \quad \Rightarrow \quad \xi_i \rightarrow 0\]
...
即$f_0-af \in X_0$，则:
\[\|f_0-(f_0-af)\|=|a|\cdot\|f\| \geq d(f_0,X_0)=1 \quad \Rightarrow \quad \left|\int_0^1f_0(x)\dd x\right\||f\| \geq \left|\int_0^1f(x)\dd x\right|\]
