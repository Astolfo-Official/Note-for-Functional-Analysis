...
\end{theorem}
\begin{proof}充分性：由范数强弱性可知，$\exists \, c>0$满足$0 \leq ||x_n||_1 \leq c||x_n||_2 \to 0$，可以推出$||x_n||_1 \to 0$。\\
必要性：采用反证法，如若不然，则对$\forall k \in \mathbb{N} \ , \ \exists \, x_k \in X$满足$||x_k||_1>k||x_n||_2$。\\
令：
\[y=\frac{x_k}{||x_k||_1}\]
则有：
...
利用算子范数的定义（xxx）和有界线性算子都是连续的（xxx），可知$\alpha \geq ||T^{-1}||$。同样的，考虑函数$f$:
