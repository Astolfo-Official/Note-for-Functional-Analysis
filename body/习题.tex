...
\[\mathop \text{sup}\limits_{t \in [t_0-\delta,t_0+\delta]}|T(x(t))-s_0|=\mathop \text{sup}\limits_{t \in [t_0-\delta,t_0+\delta]}|f(t,x(t))-s_0|=\mathop \text{sup}\limits_{t \in [t_0-\delta,t_0+\delta]}|f_s(t,\xi(t))|\cdot|x-s_0| \leq \frac{\delta}{2}\]
\end{itemize}
计算范数$d(Tx,Ty)$：
\[d(Tx,Ty)=\mathop \text{sup}\limits_{t \in [t_0-\delta,t_0+\delta]}|f(t,x(t))-f(t,y(t))|=\mathop \text{sup}\limits_{t \in [t_0-\delta,t_0+\delta]}|f_s(t,\xi(t))|\cdot|x(t)-y(t)| \leq \theta d(x,y)\]
...
\end{proposition}
\begin{proof}
利用中值定理对$\forall \varepsilon>0 \ , \ x(t) \in A \ , \ t_1,t_2 \in [0,1]$满足$|x(t_1)-x(t_2)|=|x'(\xi)|\cdot|t_1-t_2| \leq K|t_1-t_2|$，取$|t_1-t_2|<\varepsilon/K$即得$|x(t_1)-x(t_2)|<\varepsilon$，$A$等度连续得证。
\end{proof}
...
则对$\forall x \neq y \in [0,1]$：
\[|Tx-Ty|=\left|\frac{x}{1+x}-\frac{y}{1+y}\right|=\frac{|x-y|}{|1+x|\cdot|1+y|}<|x-y|\]
...
\[\begin{aligned}
\sum_{i=1}^n|x_i+y_i|^p&=\sum_{i=1}^n|x_i+y_i|\cdot|x_i+y_i|^{p-1} \leq \sum_{i=1}^n|x_i|\cdot|x_i+y_i|^{p-1}+\sum_{i=1}^n|y_i|\cdot|x_i+y_i|^{p-1}\\
