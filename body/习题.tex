...
\paragraph*{2.}证明:在$(X,d)$中，$\overline{A}=A \ \Leftrightarrow \ A^c$是开集\\
Proof ($\overline{A}=A \ \Rightarrow \ A^c$是开集):
...
Proof ($A^c$是开集$ \ \Rightarrow \ \overline{A}=A$):
...
\[\Rightarrow \ \exists \, \{x_{n_k}\} \ (k=1,2,\cdots) \quad \text{s.t.} \quad \overline{B}_{\frac{1}{2^{k+1}}}(x_{n_{k+1}}) \subset \overline{B}_{\frac{1}{2^k}}(x_{n_k})\]
\[\text{由题设} \exists \,! \, x \in \bigcap_{k=1}^{\infty}\overline{B}_{\frac{1}{2^k}}(x_{n_k}) \ , \ d(x_{n_k},x) \to 0 \ (n \to \infty) \ \Rightarrow \ d(x_n,x) \to 0 \ (n \to \infty)\]
...
$(X,d)$完备,由$Baire$定理，至少存在一个$M$使得$E_M$不是疏集，即存在一个开集$U \subset \overline{E}_M$
