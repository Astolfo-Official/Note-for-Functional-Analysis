...
\begin{equation*}
    \begin{aligned}
        ...
        & \leq \int_Ex^2(t)\dd t+\int_Ey^2(t)\dd t+2\int_E|x(t)|\cdot|y(t)|\dd t \leq \int_Ex^2(t)\dd t+\int_Ey^2(t)\dd t+2\sqrt{\int_Ex^2(t)\dd t}\sqrt{\int_Ey^2(t)\dd t} \\
...
介绍了距离空间以及距离的概念，我们可以很自然地引入一般度量意义下的收敛。
\begin{proposition}
\begin{itemize}
...
\end{theorem}
\begin{proof}利用 Weierstrass 定理，对$\forall P_n(x)$，存在有理多项式列$\{q_{n,i}(x)\}_{i=1}^{\infty}$使得$q_{n,i}(x) \rightrightarrows P_n(x) \ (i \rightarrow \infty)$，则
...
设$\{f_n\}$是测度空间$(S,\Sigma,\mu)$上的实值非负可测函数列，则
\[\int_E\varliminf_{n \to \infty}f_n\dd\mu \leq \varliminf_{n \to \infty}\int_Ef_n\dd\mu\]
...
\[d(T(x(t)),T(y(t))) \leq \lambda\mathop {\text{sup}}\limits_{t \in [a,b]}\int_a^b|K(t,s)|\cdot|x(s)-y(s)|\dd s \leq \lambda\mathop {\text{sup}}\limits_{t \in [a,b]}|K(t,s)|d(x,y)(b-a)\equiv\Gamma(b-a)\]
当$\Gamma<1$时，$T$为压缩映射：
...
\end{theorem}
这里我们规定符号$f^{-1}$仅表示原像$f:X \to Y \ (A \subseteq Y), \ f^{-1}(A)=\{x \in X \ , \ f(x) \in A\}$。
\begin{example}
...
\end{proof}
\begin{theorem}
    $T_2$空间[命题\ref{proposition:flgl}]的紧子集是闭集。
\end{theorem}
\begin{proof}
...
\begin{proposition}[分离公理]\label{proposition:flgl}
\begin{itemize}
...
\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.3]{./fig/2.2.2.png}
\end{figure}
...
\end{theorem}
\begin{proof}
先证必要条件：紧$ \ \Rightarrow \ $列紧闭，上面已经证明了紧集是闭集，采用反证法，假设存在$\{y_n\} \subset A$，它的任意子列不收敛于$A$中，即$\forall x \in A \ , \ \exists \, r_x>0 \ , \ B_{r_x}(x) \cap \{y_n\}=\{y_{n_1},y_{n_2},\cdots,y_{n_k}\} \ (k<\infty)$。显然$\{B_{r_x}(x)\}$是$A$的一个开覆盖，由$A$的紧性，存在一个有限子覆盖$\{B_{r_{x_i}}(x_i)\}\cap \{y_n\}=\{y_{n_1},y_{n_2},\cdots,y_{n_j}\} \ (j<\infty)$，因为$\{y_n\} \subset A$是柯西列，其个数不是有限个，故矛盾。下面给出证明充分性的几个引理。
...
现在证充分条件：列紧闭$ \ \Rightarrow \ $紧。设$A$是是列紧闭集，可知$A$是全有界的[引理\ref{lemma:ljqyj}]，且其任意开覆盖$\{G_{\alpha}\}$必有可数子覆盖$\{G_{\alpha_i}|i=1,2,\cdots\}$[引理\ref{lemma:kskfg}]。采用反证法，设$A$不是紧的，则存在一个$\{G_{\alpha}\}$的无有限子覆盖：
...
\begin{theorem}\label{theorem:ybljqyj}
完备距离空间中的子集是紧集当且仅当它是全有界闭集。
\end{theorem}
\begin{proof}
由定理\ref{theorem:jlj}和引理\ref{lemma:ljqyj}可知必要性显然，下证充分性，只需证明完备距离空间中全有界集是列紧的。设$A$全有界，$\{x_n\} \subset A$，存在$A$的有限$1$-网$N_1$，满足$\exists \, x \in N_1$，$B_1(x) \cap \{x_n\}$包含无数多个点，取其中一个记为$x_{n_1}$，显然$B_1(x) \cap A$也是全有界的，存在$B_1(x) \cap A$的有限$1/2$-网$N_2$，同上可得新的点$x_{n_2}$，由归纳得，存在点列$\{x_{n_k}\}$，满足$d(x_{n_k},x_{n_{k+m}})<2^{-k}$，进而可知$\{x_{n_k}\}$是柯西列。由完备性，$\exists \, x \in A$使得$x_{n_k} \to x$。
\end{proof}
...
\end{example}
\begin{proof}
$|x(t_1)-x(t_2)|=|x'(\xi)|\cdot|t_1-t_2| \leq M|t_1-t_2| \quad \xi \in (t_1,t_2) \ : \ |t_1-t_2| \to 0 \ , \ |x(t_1)-x(t_2)| \to 0$
\end{proof}
...
\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.25]{./fig/2.2.3-2.png}
\end{figure}
...
&<\frac{\varepsilon}{5}+\frac{\varepsilon}{5}+|f(t_i)-f(t)|<\frac{2}{5}\varepsilon+|f(t_i)-f(t_{i+1})|\\
