...
\item 2. $\dim\ker T=\dim\ker T^*<+\infty$；
\item 3. $R(T)=(\ker T^*)^{\perp}$，$R(T^*)=^{\perp}(\ker T)$。
\end{itemize}
\end{theorem}
\begin{remark}
\quad 由二三点可知，紧算子$A$使得$T$还原到有限维，$T$的单射等价于满射，且$T$不是单的也可以有类似结论。该定理的证明极其复杂，我们先分几步证明$T$是单射的情况，即$\ker T=\{0\}$，然后推广到$T$非单射的情况。
\end{remark}
\begin{proof}
对定理的第一点，要证明$\sigma(T) = \sigma(T^*)$，等价于证明$\rho(T) = \rho(T^*)$，也即对$\forall \lambda \in \rho(T)$证明$(\lambda\mathbbm{I}-T)^{-1}\in\mathscr{B}(X) \ \Leftrightarrow \ (\lambda\mathbbm{I}-T^*)^{-1}\in\mathscr{B}(X)$，进而等价于证明$T^{-1}\in\mathscr{B}(X) \ \Leftrightarrow \ (T^*)^{-1}\in\mathscr{B}(X)$。当然还要证$R(T)=X$。
...
因此$\{y_n\} \subset R(T)$是柯西列，记$y_n \to y$，显然有$Tx_n \to Tx \in R(T)$，考虑：
\[||Tx_n-y|| \leq ||Tx_n-y_n||+||y_n-y||=||y_n-y|| \to 0 \quad \Rightarrow \quad y=Tx \in R(T)\]
因此$R(T)$中的任意柯西列$\{y_n\}$都收敛到其自身，故$R(T)$是闭的。使用反证法证明$R(T)=X$，则$\exists \, x_0 \in X-R(T)$，由 Hahn-Banach 定理推论[命题\ref{proposition:hb3}]，$\exists \, f \in X^*$满足$f(x_0)=||x_0||\neq 0$且对$\forall y \in R(T)$都有$f(y)=0$，即对$\forall x \in X \ , \ y=Tx \in R(T)$都有$f(y)=f(Tx)=(T^*f)x=0$，因此$T^*f=0$即$f \in \ker T^*$，但是上面证明了$T^*$可逆，因此$\ker T^*=\{0\}$，因此$f=0$，得出矛盾，故$R(T)=X$，定理第一点得证。
\end{proof}
