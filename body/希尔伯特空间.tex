...
\end{definition}
\begin{proposition}[线性空间的正交分解]\label{proposition:zjfj}
设$X$是赋范线性空间，$M \subset X$是闭凸子空间，若$\forall x \in X \ , \ \exists \, x_0 \in M \ , \ y \in M^{\perp}$满足$x=x_0+y$，记为$H=M \oplus M^{\perp}$。
\end{proposition}
\begin{theorem}
设$X$是赋范线性空间，则命题\ref{proposition:zjfj}成立当且仅当$X$与一个 Hilbert 空间同构。
\end{theorem}
\begin{definition}
设$X$是赋范线性空间，$M \subset X$是闭子空间
\end{definition}
