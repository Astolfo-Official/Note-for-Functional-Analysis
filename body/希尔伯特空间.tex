...
\newpage
\begin{example}\label{exmaple:laplace_equation}
...
设$1\leq p<\infty$且$\Omega\subset\mathbb{R}^n$为有界区域，则对$\forall u\in W_0^{1,p}(\Omega)$存在正常数$C=C(p,\Omega)$使得：
...
\[|F(v)|=\left|\int_{\Omega}f \cdot v \dd{x}\right| \leq \|f\|_{L^2(\Omega)} \cdot \|v\|_{L^2(\Omega)} \leq C\|f\|_{L^2(\Omega)} \cdot \|Dv\|_{L^2(\Omega)}\leq C\|f\|_{L^2(\Omega)} \cdot \|Dv\|_{H_0^1(\Omega)}<\infty\]
